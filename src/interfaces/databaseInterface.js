...
function databaseProcess(direction) {
  ...
  if (direction === 'pull') {
    ...
    conn.on('ready', () => {
      conn.exec('wp db export ' + config.remote.path + settings.database.file, (err, stream) => {
        if (err) {
        ...
        stream.on('close', (code, signal) => {
          ...
          conn.end();
          rsyncPull(config.remote.path + settings.database.file , config.local.path);

          const importCommand = 'wp db import ' + config.local.path + settings.database.file;
          const deleteCommand = 'rm ' + config.remote.path + settings.database.file;
          ...
          conn.exec(importCommand, (err, stream) => {
            ...
            stream.on('close', (code, signal) => {
              ...
              conn.exec(deleteCommand, (err, stream) => {
                if (err) {
                  console.error('Error deleting the database file:', err);
                  conn.end();
                  return;
                }
                ...
                stream.on('close', (code, signal) => {
                  ...
                  console.error('Error deleting the database file:', data.toString());
                  conn.end();
                });
              });
            }).stderr.on('data', (data) => {
              console.error('Error importing the database:', data.toString());
              conn.end();
            });
          });
        }).stderr.on('data', (data) => {
